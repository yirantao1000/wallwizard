  ...
  tf2_web_republisher__msg__TFArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    tf2_web_republisher__msg__TFArray * data =
      (tf2_web_republisher__msg__TFArray *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!tf2_web_republisher__msg__TFArray__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          tf2_web_republisher__msg__TFArray__fini(&output->data[i]);
        }
        return false;
      }
    }
