...
class LocateArUcoTag(hm.HelloNode):
    ...
    def __init__(self):
        ...
        self.min_tilt_position = -0.5
        self.tilt_num_steps = 1
        self.tilt_step_size = pi/10

        # Define the head actuation rotational velocity
        self.rot_vel = 0.2 # radians per sec
        
        self.artag_trans = None
                ...
                while True:
                    try:
                        ...
                        trans_cam = self.tf_buffer.lookup_transform(
                                                                'camera_link', tag_name,
                                                                rospy.Time())
                        self.artag_trans = trans_cam
                        rospy.loginfo("tag to camera: \n%s", trans_cam)
                        depth = self.artag_trans.transform.translation.x
                        x = self.artag_trans.transform.translation.z
                        y = self.artag_trans.transform.translation.y
                        if abs(x/depth) > 0.1 or abs(y/depth) > 0.3:
                            rospy.loginfo("== visual servo adjustment ==")
                            delta_pan = -0.1*x/depth
                            delta_tilt = 0.05*y/depth
                            pan_command = {'joint': 'joint_head_pan', 'delta': delta_pan}
                            tilt_command = {'joint': 'joint_head_tilt', 'delta': delta_tilt}
                            self.send_command(pan_command)
                            self.send_command(tilt_command)
